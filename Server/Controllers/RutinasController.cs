...
namespace GreenCoinHealth.Server.Controllers
{
    ...
    public class RutinasController : ControllerBase
    {
        ...
        {
            return await _context.Rutinas.Include(r => r.Ejercicios).ToListAsync();
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<Rutina>> GetRutina(int id)
        {
            var rutina = await _context.Rutinas.Include(r => r.Ejercicios).FirstOrDefaultAsync(r => r.RutinaId == id);
